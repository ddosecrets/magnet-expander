...
# From https://techspree.net/torrent-tracker-list-updated/
TRACKERS = [
...
if __name__ == "__main__":
	if( len(sys.argv) != 2 ):
		...
		sys.exit(1)
	try:
		magnet = Magnet.from_string(sys.argv[1])
	except URLError:
		sys.stderr.write("Magnet link '%s' appears to be invalid\n" % sys.argv[1])
		sys.exit(1)
	except MagnetError:
		sys.stderr.write("Magnet link '%s' appears to be invalid\n" % sys.argv[1])
		sys.exit(1)

	# Get the set of current trackers, add any from our global list that aren't there already
	trackers = set(magnet.tr)
	for tracker in TRACKERS:
		if( tracker not in trackers ):
			magnet.tr.append(tracker)
	print(magnet)
